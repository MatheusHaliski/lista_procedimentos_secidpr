...
import styles from './FAQProcedimentos.module.css';
...
export default function FAQProcedimentos() {
  ...
  const goToPrevious = useCallback(() => {
    ...
    setSelectedId(filteredData[(selectedIndex - 1 + filteredData.length) % filteredData.length].id);
  }, [filteredData, selectedIndex]);

  const goToNext = useCallback(() => {
    if (filteredData.length <= 1 || selectedIndex < 0) return;
    setSelectedId(filteredData[(selectedIndex + 1) % filteredData.length].id);
  }, [filteredData, selectedIndex]);
  ...
  return (
    <section className={styles.section}>
      ...
      <input
        ...
        placeholder="Buscar por pergunta, categoria ou tag…"
        className={styles.searchInput}
        aria-label="Buscar perguntas"
      />

      <div className={styles.categoryRow} role="group" aria-label="Filtrar por categoria">
      ...
      <p className={styles.count}>
        Exibindo {filteredData.length} de {faqData.length} perguntas
      </p>
          ...
          return (
            <li key={item.id}>
              <button
                ...
                aria-label={`Abrir resposta: ${item.pergunta}`}
              >
                <div className={styles.cardTop}>
                  <div className={styles.cardLeft}>
                    ...
                    <span className={styles.questionId}>Pergunta #{item.id.toString().padStart(2, '0')}</span>
                  </div>
                  <span className={styles.categoryTag}>{item.categoria}</span>
                </div>
                <p className={styles.questionText}>{item.pergunta}</p>
                <div className={styles.tagList} aria-label="Tags">
                  ...
                  {hiddenCount > 0 && <span className={styles.tagExtra}>+{hiddenCount}</span>}
                </div>
              </button>
            </li>
          );
      ...
      {filteredData.length === 0 && (
        <div className={styles.empty}>
          ...
          <p className={styles.emptyText}>
            Nenhuma pergunta encontrada para <strong>{searchTerm || activeCategory}</strong>
          </p>
          <p className={styles.emptyHint}>Tente buscar por outro termo ou selecione outra categoria.</p>
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <div className={styles.modalHeader}>
              <span className={styles.modalCategoryChip}>{selectedItem.categoria}</span>
              <h2 id="faq-modal-title" className={styles.modalTitle}>
                {selectedIndex + 1}. {selectedItem.pergunta}
              </h2>
            </div>
              ...
              <div className={styles.modalTagsSection}>
                ...
                <div className={styles.modalTagList}>
                  {selectedItem.tags.map((tag) => (
                    <span key={`${selectedItem.id}-${tag}`} className={styles.modalTag}>
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className={styles.modalFooter}>
              <span className={styles.navInfo}>
                Pergunta {selectedIndex + 1} de {filteredData.length}
              </span>
